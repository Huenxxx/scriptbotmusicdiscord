...
from music_sources import (
    is_spotify_url, extract_spotify_id,
    get_spotify_playlist, get_spotify_album,
)
...
# guild_id → lista de resultados pendientes de !select
...
async def get_or_create_player(ctx: commands.Context) -> LavalinkPlayer | None:
    ...
    y devuelve (o crea) el LavalinkPlayer del servidor.
    """
...
async def on_ready():
    ...
    try:
        ...
        print('   El bot funcionará con FFmpeg como fallback.')
    print('=' * 60)


@bot.event
async def on_wavelink_track_end(payload: wavelink.TrackEndEventPayload):
    """Cuando termina una pista, reproduce la siguiente automáticamente."""
    player = payload.player
    if player is None:
        return
    guild_id = player.guild.id
    lp = lava_players.get(guild_id)
    ...
    try:
        ...
        if is_spotify_url(query):
            ...
            if tipo in ('playlist', 'album'):
                # Lavalink puede cargar playlists de Spotify nativamente
                playlist = await lavalink_load_playlist(query)
                if playlist and playlist.tracks:
                    label = 'playlist' if tipo == 'playlist' else 'álbum'
                    await ctx.send(f'📋 Cargando {len(playlist.tracks)} canciones del {label} de Spotify...')
                    for track in playlist.tracks:
                        lp.queue.append(track)
                    if not lp.wl.playing:
                        await lp.play_next()
                    await ctx.send(f'✅ {len(playlist.tracks)} canciones añadidas')
                else:
        ...
        # Una sola canción o URL directa → reproducir directo
...
# ─── Música local (FFmpeg, no requiere Lavalink) ─────────────────────────────
...
async def ownplay(ctx: commands.Context, *, query: str = None):
    ...
    # Conectar y reproducir con FFmpeg
    vc = ctx.voice_client
    if vc is None:
        vc = await ctx.author.voice.channel.connect()

    if vc.is_playing() or vc.is_paused():
        await ctx.send(f'⚠️ El bot ya está reproduciendo. Usa `!stop` primero para música local.')
        return
...
if __name__ == '__main__':
    if not DISCORD_TOKEN or DISCORD_TOKEN == 'PEGA_TU_TOKEN_AQUI':
        ...
        try:
            print('🤖 Iniciando bot con Lavalink...')
